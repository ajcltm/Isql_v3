...
class Test_select_from(unittest.TestCase):
    ...
    def test_with_subquery(self):
        ...
        sql = querySql.QuerySql('sq_1').set_table(tableName='sq_1', subQuery=subquery).select('table1.age','sq_1.name').export_sql()
        print('='*200, sql, '='*150, sep='\n')
    ...
    def test_specific_fields_with_subquery(self):
        table1 = 'table1'
        table2 = 'table2'
        subquery = querySql.QuerySql().set_table(tableName=table1).select('table1.name', 'table1.age').from_(table1)
        sql = querySql.QuerySql().set_table(tableName='sq_1', subQuery=subquery).select('table2.name', 'sq_1.age').from_(table2,'sq_1').export_sql()
        print('='*200, sql, '='*150, sep='\n')
    ...
    def test_where_with_two_subquery(self):
        ...
        table2 = 'table2'
        table3 = 'table3'
        subquery = querySql.QuerySql(table=table1).set_table(tableName=table1).select('name', 'age')
        subquery2 = querySql.QuerySql().set_table(tableName='sq_1', subQuery=subquery).select('table2.name', 'sq_1.age').from_(table2,'sq_1').where(sq_1_name = '="kim"')
        sql = querySql.QuerySql().set_table(tableName=table3).set_table(tableName='sq_2', subQuery=subquery2).select('table3.name', 'sq_2.age').from_(table3,'sq_2').where(table3_age='>10').export_sql()
        print('='*200, sql, '='*150, sep='\n')
    ...
    def test_join_with_subquery(self):
        table1 = 'table1'
        table2 = 'table2'
        subquery = querySql.QuerySql().set_table(table1).select('table1.id').from_(table1).where(table1_name='="kim"')
        sql = querySql.QuerySql().set_table(tableName='sq', subQuery=subquery).set_table(table2).select('table2.name', 'table3.age').from_(table2).join('sq', how='left', table2_age='=sq.age').export_sql()
        print('='*200, sql, '='*150, sep='\n')
    ...
    @unittest.skip('')
    def test_groupby_with_subquery(self):
        table1 = 'table1'
        table2 = 'table2'
        table3 = 'table3'
    ...
    def test_orderBy_with_subQuery(self):
        ...
        table3 = 'table3'
        subquery = querySql.QuerySql().set_table(table1).select('table1.name', 'table2.age').from_(table1).join(table2, how='left').on(table1_age='=table2.age')
        sql = querySql.QuerySql().set_table(tableName='sq', subQuery=subquery).set_table(table3).select('table3.id').from_('sq').where(table3_age='>10').group_by('sq.name', 'sq.age').max('sq.age').min('sq.age').order_by('sq.name', 'sq.age').export_sql()
        print('='*200, sql, '='*150, sep='\n')
